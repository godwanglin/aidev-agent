...
```json
{
  ...
  "transcriptPath": "/path/to/workspace/.aidev/aidev/transcript.jsonl",
  "artifactDirectoryPath": "/path/to/workspace/.aidev/aidev/artifacts",
  "modelName": "auto"
}

> [!NOTE]
> The `transcriptPath` and `artifactDirectoryPath` are product-specific. The example above uses `aidev` (for Aidev Desktop 2.0). Depending on the interface you are using, the directory name will differ:
*   **CLI**: `aidev-cli/`
*   **Aidev Desktop 2.0**: `aidev/`
*   **IDE**: `aidev-ide/`
```
